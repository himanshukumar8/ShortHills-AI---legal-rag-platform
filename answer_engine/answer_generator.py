...
from answer_engine.response_validator import validate_answer_schema
...
class AnswerGenerator:
    ...
    def generate_answer(self, prompt: GeneratedPrompt) -> dict:
        ...
        try:
            ...
            logger.info(f"Raw Length        : {len(llm_res.raw_response)}")
            # =========================================
    ...
    def _save_report(self, report_data: dict) -> None:
        ...
        report_path = (
            self.config.report_dir / "llm_generation_report.json"
        )
        ...
        logger.info(
            f"Answer generation report written to {report_path}"
        )